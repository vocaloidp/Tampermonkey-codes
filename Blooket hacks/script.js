...
alert("It seems the GitHub is either blocked or down.");
    ...
    function runcode(code){new Eval(code).evaluate(new Env(env));}
async function runcheat(){
    /*if(event.key === '`')runcode(await getImgText("https://raw.githubusercontent.com/DannyDan0167/Blooket-Cheats-Plus/main/csptest/out/global/useanyblook.png?"));
    if(event.key === '/')runcode(await getImgText("https://raw.githubusercontent.com/DannyDan0167/Blooket-Cheats-Plus/main/csptest/out/global/crashgame.png?"));
    if(event.key === "'")runcode(await getImgText("https://raw.githubusercontent.com/DannyDan0167/Blooket-Cheats-Plus/main/csptest/out/global/freezescoreboard.png?"));
    if(event.key === '=')runcode(await getImgText("https://raw.githubusercontent.com/DannyDan0167/Blooket-Cheats-Plus/main/csptest/out/crypto/crashpassword.png?"));*/
}
async function getCheats(){return JSON.parse((await getImgText("https://raw.githubusercontent.com/DannyDan0167/Blooket-Cheats-Plus/main/csptest/out/cheats.png?"+Date.now())).split("\x00")[0]);}

const blookScriptGUI = document.createElement('div');
blookScriptGUI.id = 'blookScriptGUI';
blookScriptGUI.innerHTML = `
        <select id="options" style="visibility: hidden;">
        <option></option>
        </select>
    </div>`
...
runcheat();
    ...
    const cheat = (async () => {
        ...
        const guiWrapper = createElement("div", {
            ...
            (gui = createElement("div", {
                ...
                (cheatContainer = createElement("div", {
                    ...
                    createElement("a", {
                        ...
                        innerHTML: `<div style="background: rgba(0,0,0,.25); border-radius: 5px; width: 100%; height: 100%; left: 0; top: 0; position: absolute; transform: translateY(2px); width: 100%; transition: transform .6s cubic-bezier(.3,.7,.4,1)"></div>
            ...
            <div style="font-weight: 400; background-color: rgb(11, 194, 207); color: white;  flex-direction: row; align-items: center; justify-content: center; text-align: center; padding: 5px; border-radius: 5px; transform: translateY(-4px); transition: transform .6s cubic-bezier(.3,.7,.4,1)">
            <div style="font-family: Titan One, sans-serif; color: white; font-size: 26px; text-shadow: 2px 2px rgb(0 0 0 / 20%); height: 40px; padding: 0 15px; flex-direction: row; align-items: center; justify-content: center">
